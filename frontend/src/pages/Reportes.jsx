...
import { Card, Row, Col, Statistic, Typography, Table, Tag, DatePicker, Button, Spin, Progress, Tabs, Space, Dropdown } from 'antd'
import { FileTextOutlined, CheckCircleOutlined, ClockCircleOutlined, ReloadOutlined, DownloadOutlined } from '@ant-design/icons'
...
export default function Reportes() {
  ...
  const descargarPDFGeneral = () => {
    ...
    autoTable(doc, {
      ...
      columnStyles: { 4: { textColor: [82, 196, 26] } },
  ...
  // Datos agregados para resumen general
  const dataDocumentosResumen = [
    {
      name: 'Documentos',
      Subidos: documentos?.documentos?.reduce((sum, d) => sum + d.total_subidos, 0) ?? 0,
      Aprobados: documentos?.documentos?.reduce((sum, d) => sum + d.aprobados, 0) ?? 0,
      Observados: documentos?.documentos?.reduce((sum, d) => sum + d.observados, 0) ?? 0,
    }
  ]

  const dataFirmasResumen = [
    {
      name: 'Firmas',
      Completadas: firmas?.por_rol?.reduce((sum, r) => sum + r.firmadas, 0) ?? 0,
      Pendientes: firmas?.por_rol?.reduce((sum, r) => sum + r.pendientes, 0) ?? 0,
      Rechazadas: firmas?.por_rol?.reduce((sum, r) => sum + r.rechazadas, 0) ?? 0,
    }
  ]

  const dataNotificacionesResumen = notificaciones?.por_tipo?.map(n => ({
    name: n.tipo_notificacion.replace(/_/g, ' '),
    Enviadas: n.enviados,
    Fallidas: n.fallidos,
  })) ?? []
      ...
      children: (
        <div>
          <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
            ...
            <Col xs={24} sm={12} md={8}>
              <Card style={{ borderRadius: 12, border: '1px solid #1677ff', background: '#e6f4ff' }}>
                <Statistic title="Dias promedio"
                  value={resumen?.dias_promedio_certificacion ?? 'N/A'}
                  suffix={resumen?.dias_promedio_certificacion ? 'dias' : ''}
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} lg={12}>
              <Card title="Por nivel de formación" style={{ borderRadius: 12 }}>
                <div className="chart-container">
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={dataTipos} margin={{ bottom: 30 }}>
          ...
          <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
            <Col xs={24} lg={12}>
              <Card title="Estado de documentos" style={{ borderRadius: 12 }}>
                <div className="chart-container">
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={dataDocumentosResumen}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="name" />
                      <YAxis />
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="Subidos" fill="#1677ff" />
            ...
            <Col xs={24} lg={12}>
              <Card title="Estado de firmas" style={{ borderRadius: 12 }}>
                <div className="chart-container">
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={dataFirmasResumen}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="name" />
                      <YAxis />
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="Completadas" fill="#52c41a" />
          ...
          {dataNotificacionesResumen.length > 0 && (
            <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
              <Col xs={24}>
                <Card title="Estado de notificaciones" style={{ borderRadius: 12 }}>
                  <div className="chart-container">
                    <ResponsiveContainer width="100%" height={280}>
                      <BarChart data={dataNotificacionesResumen} margin={{ bottom: 50 }}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="name" angle={-30} textAnchor="end" height={70} />
                        <YAxis />
                        <Tooltip />
                        <Legend />
                        <Bar dataKey="Enviadas" fill="#52c41a" />
                        <Bar dataKey="Fallidas" fill="#ff4d4f" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </Card>
        ...
        <div>
          <Card style={{ borderRadius: 12, marginBottom: 16 }}>
            <div className="period-controls">
              <RangePicker value={fechas} onChange={setFechas} format="YYYY-MM-DD" style={{ minWidth: 250 }} />
          ...
          <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
            {[
              ...
              { label: 'Con obs.', value: periodo?.resumen?.con_observaciones ?? 0, color: '#ff4d4f' },
          ...
          <Card title="Solicitudes del período" style={{ borderRadius: 12 }}>
            <Table dataSource={periodo?.solicitudes} columns={columnasSolicitudes}
              rowKey="id" size="small" scroll={{ x: 700 }} pagination={{ pageSize: 10, responsive: true }} />
          </Card>
        </div>
      )
    },
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24}>
            <Card title="Firmas por rol" style={{ borderRadius: 12, marginBottom: 16 }}>
              <div className="chart-container">
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={dataFirmasPorRol} margin={{ bottom: 50 }}>
                    <CartesianGrid strokeDasharray="3 3" />
                ...
                {firmas?.por_rol?.map(r => (
                  <div key={r.rol} style={{ marginBottom: 8 }}>
                    <div className="progress-row">
                      <Text style={{ fontSize: 13 }}>{r.rol}</Text>
                      <Text type="secondary" className="progress-percentage">{r.porcentaje_firmado}%</Text>
                    </div>
    ...
    },
    {
      ...
      children: (
        <Card title="Estado de documentos por tipo" style={{ borderRadius: 12 }}>
          <div className="chart-container">
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={dataDocumentos} margin={{ bottom: 50 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="name" angle={-45} textAnchor="end" height={80} />
                <YAxis />
                <Tooltip
          ...
          <Table
            ...
            scroll={{ x: 700 }} style={{ marginTop: 16 }}
          />
        </Card>
      )
    },
                ...
                {notificaciones?.por_tipo?.map(n => (
                  <Col xs={24} sm={12} lg={8} key={n.tipo_notificacion}>
                    <Card size="small" style={{ borderRadius: 8, height: '100%' }}>
                      ...
                      <Progress percent={n.porcentaje_exito} strokeColor="#52c41a" size="small" />
                    </Card>
                  </Col>
                ))}
              </Row>
    ...
    <div>
      <style>{`
        ...
        @media (max-width: 576px) {
          ...
          .reportes-header .ant-space .ant-btn {
            flex: 1;
            min-width: 100px;
          }
        }
        ...
        @media (max-width: 576px) {
          ...
          .period-controls .ant-picker-range {
            width: 100%;
          }
          .period-controls .ant-btn {
            width: 100%;
          }
        }
        .ant-table {
